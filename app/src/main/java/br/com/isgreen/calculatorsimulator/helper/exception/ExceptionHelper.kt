package br.com.isgreen.calculatorsimulator.helper.exception
...
class ExceptionHelper {
    ...
    companion object {
        ...
        fun getMessage(exception: Throwable): Any {
            return getMessage(exception, true, R.string.there_was_a_fail_try_again)
        }

        fun getMessage(exception: Throwable, readApiMessage: Boolean): Any {
            return getMessage(exception, readApiMessage, R.string.there_was_a_fail_try_again)
        }
        ...
        fun getMessage(
            ...
            val message: Any? = when (exception) {
                is HttpException -> getErrorMessage(exception, readApiMessage)
                is UnknownHostException ->
                    R.string.no_internet_signal
                is SocketTimeoutException ->
                    R.string.no_server_response
                is BaseValidationException ->
                    exception.message
                else ->
                    defaultMessageRes
            }
            ...
            if (readApiMessage) {
                try {
                    exception.response()?.errorBody()?.let {
                        val error = JSONObject(convertStreamToString(it.byteStream()))
                        return when {
